...
namespace DINPUT_key_switcher
{
    public partial class Form1 : Form
    {
        ...
        private byte[] code1= new byte[] {0x8B,0xC3,0x83,0xE8};
        private byte[] code2 = new byte[] {0x0F,0x85,0x0C,0x00,0x00,0x00,0xC7,0x45,0x10};
        private byte[] code3 = new byte[] {0x00,0x00,0x00,0xE9};
        private byte[] codef = new byte[] {0xFF,0x75,0x10,0xFF,0x75,0x0C,0xE9};
        ...
        const int codeAdress=0x213c3;   //Offset del DLL casi al final de este, y a partir del cual inyectamos nuestro assembly
        ...
        {
            try {
                archivo = new FileStream("dinput.dll", FileMode.Open, FileAccess.ReadWrite);
            }
            catch (Exception)
            {
                System.Windows.Forms.MessageBox.Show("customized dinput.dll not found. You have to extract the exe and the dll in the same folder.");
            ...
            archivo.Seek(codeAdress + 14, SeekOrigin.Begin);
            ...
            {
                archivo.Seek(-14, SeekOrigin.Current);
                archivo.Read(leido, 0, 4);
                ...
                archivo.Seek(29, SeekOrigin.Current);
                archivo.Read(leido, 0, 4);
                e.final_key = BitConverter.ToInt32(leido, 0);
            }
        ...
        {
            if (entradas.Count>=24)     //Tenemos espacio limitado en el DLL
            {
        ...
        private void SaveAndExit(object sender, EventArgs e)    //Modifica el DLL
        {
            ...
            {
                archivo.Write(code1, 0, 4);
                archivo.WriteByte((byte)((entrada)entradas[i]).initial_key);
                archivo.Write(code2, 0, 9);
                archivo.WriteByte((byte)((entrada)entradas[i]).final_key);
                archivo.Write(code3, 0, 4);
                archivo.Write(BitConverter.GetBytes((entradas.Count-i-1)*23), 0, 4);    //Instrucción "jmp" personalizada, siempre debe aputar al codigo final codef
            }
            archivo.Write(codef, 0, 7);     //codigo de finalización
            uint n = 0xFFFE4082;
            n -= (uint)(23*i);
            archivo.Write(BitConverter.GetBytes(n), 0, 4);                  //Instrucción "jmp" personalizada, vuelve a donde estabamos al inicio
            while (archivo.Position<archivo.Length) archivo.WriteByte(0);   //Ponemos el resto de bytes a 0 hasta el final del archivo
            archivo.Close();
            System.Environment.Exit(0);
        }
        ...
        {
            System.Windows.Forms.MessageBox.Show("Dinput key switcher:\nSome games don't allows to change the keyboard map. This app generates a customized DINPUT.DLL file that must be copied to the game's directory to change the keys you use in your game.\n\nHow it works?\nThis app automatically generates the assembly code needed to change the keydown and keyup events for the configured keys, inserting the code in a empty space inside the DLL.\n\nMade in 2017 by Juanmv94\nhttp://tragicomedy-hellin.blogspot.com");
